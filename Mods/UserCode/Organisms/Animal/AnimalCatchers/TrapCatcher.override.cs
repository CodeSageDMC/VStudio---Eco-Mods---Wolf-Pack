...
// for mod ranching
 // only one type of Domesticated

        public static readonly List<string> AnimalTarget = new()
        {
            nameof(MountainGoat),
            nameof(Bison),
            nameof(Turkey),
...
// for mod ranching
    public class AnimalCatcher : TrapCatcher
    {
        public override ThreadSafeList<string> DefaultTargetSpecies => new(CatchTarget.AnimalTarget);
        public override TimeSpan NextCatchDelay                     => TimeSpan.FromMinutes(RandomUtil.Range(5, 10));

        public AnimalCatcher(User user, WorldObject obj) : base(user, obj) { }
        public AnimalCatcher() { }
    }
